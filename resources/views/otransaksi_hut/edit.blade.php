...
<script>
	...
	/////////////////////////////////////////////////////////	
			...
			$.ajax(
			{
				...
				success: function( response )
				{
					...
					for(i=0; i<resp.length; i++){
						...
						dTableBBeli.row.add([
							'<a href="javascript:void(0);" onclick="chooseBeli(\''+resp[i].NO_BUKTI+'\', \''+resp[i].TGL+'\', \''+resp[i].TOTAL+'\' , \''+resp[i].SISA+'\'   )">'+resp[i].NO_BUKTI+'<input id="pilihFaktur'+i+'" hidden value="'+resp[i].NO_BUKTI+'"></a>',
							resp[i].KODES,
							resp[i].NAMAS,
		...
		chooseBeli = function(NO_BUKTI, TGL, TOTAL, SISA){
			$("#NO_FAKTUR"+rowidBeli).val(NO_BUKTI);
			$("#TGL_FAKTUR"+rowidBeli).val(TGL);
		...
		chooseBeliArr = function(){
			...
			for (i=0 ; i<fakturArr.length ; i++) 
			{
				...
				$("#TGL_FAKTUR"+(rowidBeli+i)).val(tglArr[i]);
