                            ...
                            <table id="datatable" class="table table-striped table-border">
                                <thead>
                                    <tr>
                                        ...
                                        <th style="text-align: center;">Partisipasi</th>
								...
								@foreach ($detail as $detail)		
                                    <tr>
										...
										<td><input name="DIS[]" onclick="select()" onkeyup="hitung()" value="{{$detail->DIS}}" id="DIS{{$no}}" type="text" style="text-align: right"  class="form-control DIS text-primary"></td>                         
...
// 		 mati();
	...
	function hidup() {
		...
		for (i = 0; i <= jumlahdata; i++) {
			...
			$("#DIS" + i.toString()).attr("readonly", false);
			$("#PAR" + i.toString()).attr("readonly", false);
			$("#TGLMX" + i.toString()).attr("disabled", false);
			$("#TGLSX" + i.toString()).attr("disabled", false);
	...
	function mati() {
		...
		for (i = 0; i <= jumlahdata; i++) {
			...
			$("#KD_BRG" + i.toString()).attr("readonly", true);
			$("#NA_BRG" + i.toString()).attr("readonly", true);
			$("#DIS" + i.toString()).attr("readonly", true);
			$("#PAR" + i.toString()).attr("readonly", true);
			$("#TGLMX" + i.toString()).attr("disabled", true);
			$("#TGLSX" + i.toString()).attr("disabled", true);
    ...
    function tambah() {
		...
		html=`<tr>
				...
				<td>
		            <input name='DIS[]' onclick='select()' onblur='hitung()' value='0' id='DIS${idrow}' type='text' style='text-align: right' class='form-control DIS text-primary' required >
                </td>

				<td>
		            <input name='PAR[]' onclick='select()' onblur='hitung()' value='0' id='PAR${idrow}' type='text' style='text-align: right' class='form-control PAR text-primary' required >
                </td>

				<td>
					<input name="TGLMX[]" ocnlick='select()' id="TGLMX${idrow}" type="text" class="date form-control text_input TGLMX" data-date-format="dd-mm-yyyy" value="<?php if (isset($_POST["tampilkan"])) {
																																										} else echo 											date('00-00-0000'); ?>" onclick="select()">
				</td>	

				<td>
					<input name="TGLSX[]" ocnlick='select()' id="TGLSX${idrow}" type="text" class="date form-control text_input TGLSX" data-date-format="dd-mm-yyyy" value="<?php if (isset($_POST["tampilkan"])) {
																																										} else echo 											date('00-00-0000'); ?>" onclick="select()">
				</td>		
		...
		jumlahdata = 100;
		for (i = 0; i <= jumlahdata; i++) {
			$("#DIS" + i.toString()).autoNumeric('init', {
				aSign: '<?php echo ''; ?>', vMin: '-999999999.99'});


			$("#PAR" + i.toString()).autoNumeric('init', {
				aSign: '<?php echo ''; ?>',
				vMin: '-999999999.99'
			});	 
