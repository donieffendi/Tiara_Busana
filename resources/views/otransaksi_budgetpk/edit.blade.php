                    ...
                    <form action="{{($tipx=='new')? url('/budgetpk/store') : url('/budgetpk/update/'.$header->NO_ID.'' ) }}" method="POST" name ="entri" id="entri" >
                        ...
                        <div class="tab-content mt-3">
							...
							<div class="mt-3 col-md-12 form-group row">
								<div class="col-md-4">
									<button hidden type="button" id='TOPX'  onclick="location.href='{{url('/pantau/edit/?idx=' .$idx. '&tipx=top' )}}'" class="btn btn-outline-primary">Top</button>
									<button hidden type="button" id='PREVX' onclick="location.href='{{url('/pantau/edit/?idx='.$header->NO_ID.'&tipx=prev&buktix='.$header->NO_SP )}}'" class="btn btn-outline-primary">Prev</button>
									<button hidden type="button" id='NEXTX' onclick="location.href='{{url('/pantau/edit/?idx='.$header->NO_ID.'&tipx=next&buktix='.$header->NO_SP )}}'" class="btn btn-outline-primary">Next</button>
									<button hidden type="button" id='BOTTOMX' onclick="location.href='{{url('/pantau/edit/?idx=' .$idx. '&tipx=bottom' )}}'" class="btn btn-outline-primary">Bottom</button>
								</div>
								<div class="col-md-5">
									<button hidden type="button" id='NEWX' onclick="location.href='{{url('/pantau/edit/?idx=0&tipx=new' )}}'" class="btn btn-warning">New</button>
									<button hidden type="button" id='EDITX' onclick='hidup()' class="btn btn-secondary">Edit</button>
									<button hidden type="button" id='UNDOX' onclick="location.href='{{url('/pantau/edit/?idx=' .$idx. '&tipx=undo' )}}'" class="btn btn-info">Undo</button>
...
<script>
	...
	$('#CNT').select2({
        ...
        ajax: {
            ...
            processResults: function(data) {
                return {
                    results: data.map(item => ({
                        id: item.CNT, // The ID of the user
                        text: item.NA_CNT, // The text to display
						NA_CNT: item.NA_CNT
                    }))
                };
            },
	...
	$('#CNT').on('select2:select', function (e) {
...
// 		 mati();
    ...
    function tambah() {
		...
		html=`<tr>
				...
				<td>
		            <input name='HARGA[]' onclick='select()' onblur='hitung()' value='0' id='HARGA${idrow}' type='text' style='text-align: right' class='form-control HARGA text-primary' required readonly>
                </td>
				...
				success: function( resp )
				{
					...
					for(i=0; i<resp.length; i++){
						html+=`<tr>
									...
									<td >
										<input name='HARGA[]' onclick='select()' onblur='hitung()' id='HARGA${i}' value="0" type='text' style='text-align: right' class='form-control HARGA text-primary' required >
									</td>
									<td >
                    				...
                    				<td>
										<input name='KDLAKU[]' id='KDLAKU${i}' value="" type='text' class='form-control  KDLAKU' readonly >
									</td>
