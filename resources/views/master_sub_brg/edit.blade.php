        ...
        <div class="row">
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <form action="{{($tipx=='new')? url('/sub_brg/store/') : url('/sub_brg/update/'.$header->NO_ID ) }}" method="POST" name ="entri" id="entri" >
								...
								<div class="form-group row">
									...
									<div class="col-md-2">
										<select id="DEPT" class="form-control"  name="DEPT">
											<option value="PB" {{ ($header->DEPT == 'PB') ? 'selected' : '' }}>PB</option>
											<option value="FO" {{ ($header->DEPT == 'FO') ? 'selected' : '' }}>FO</option>
											<option value="FF" {{ ($header->DEPT == 'FF') ? 'selected' : '' }}>FF</option>
											<option value="NF" {{ ($header->DEPT == 'NF') ? 'selected' : '' }}>NF</option>
											<option value="ST" {{ ($header->DEPT == 'ST') ? 'selected' : '' }}>ST</option>
										</select>
									</div>
									<!-- tutupannya -->
								</div>
						...
						<div class="mt-3 col-md-12 form-group row">
							<div class="col-md-4">
								<button hidden type="button" id='TOPX'  onclick="location.href='{{url('/sub_brg/edit/?idx=' .$idx. '&tipx=top')}}'" class="btn btn-outline-primary">Top</button>
								<button hidden type="button" id='PREVX' onclick="location.href='{{url('/sub_brg/edit/?idx='.$header->NO_ID.'&tipx=prev&kodex='.$header->CNT )}}'" class="btn btn-outline-primary">Prev</button>
								<button hidden type="button" id='NEXTX' onclick="location.href='{{url('/sub_brg/edit/?idx='.$header->NO_ID.'&tipx=next&kodex='.$header->CNT )}}'" class="btn btn-outline-primary">Next</button>
								<button hidden type="button" id='BOTTOMX' onclick="location.href='{{url('/sub_brg/edit/?idx=' .$idx. '&tipx=bottom')}}'" class="btn btn-outline-primary">Bottom</button>
							</div>
							<div class="col-md-5">
								<button hidden type="button" id='NEWX' onclick="location.href='{{url('/sub_brg/edit/?idx=0&tipx=new')}}'" class="btn btn-warning">New</button>
								<button hidden type="button" id='EDITX' onclick='hidup()' class="btn btn-secondary">Edit</button>
								<button hidden type="button" id='UNDOX' onclick="location.href='{{url('/sub_brg/edit/?idx=' .$idx. '&tipx=undo' )}}'" class="btn btn-info">Undo</button>
							...
							<div class="col-md-3">
								...
								<!-- <button type="button" id='CLOSEX'  onclick="location.href='{{url('/sub_brg' )}}'" class="btn btn-outline-secondary">Close</button> -->
...
//////////////////////////////////////////////////////////////////////////////////////////////////
		...
		Swal.fire({
			...
			if (result.isConfirmed) {
				...
				Swal.fire({
	            	...
	            	loc = "{{ url('/sub_brg/delete/'.$header->NO_ID) }}"  ;
	...
	function CariBukti() {
		...
		var cari = $("#CARI").val();
		var loc = "{{ url('/sub_brg/edit/') }}" + '?idx={{ $header->NO_ID}}&tipx=search&kodex=' +encodeURIComponent(cari);
		window.location = loc;
