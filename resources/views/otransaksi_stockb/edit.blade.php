                            ...
                            <table id="datatable" class="table table-striped table-border">
                                <thead>
                                    <tr>
                                        ...
                                        <th style="text-align: center;">Stok Komputer</th>
                                        <th style="text-align: center;">Selisih</th>
                                        <th style="text-align: center;">Stok Riil</th>
								...
								@foreach ($detail as $detail)
                                    <tr>
										...
										<td>
                                            <input name="SALDO[]" onclick="select()" onkeyup="hitung()" value="{{$detail->saldo}}" id="SALDO{{$no}}" type="text" style="text-align: right"  class="form-control SALDO text-primary">
                                        </td>
										<td>
                                            <input name="QTY[]" onclick="select()" onkeyup="hitung()" value="{{$detail->qty}}" id="QTY{{$no}}" type="text" style="text-align: right"  class="form-control QTY text-primary">
                                        </td>
										<td>
                                            <input name="RIIL[]" onclick="select()" onkeyup="hitung()" value="{{$detail->riil}}" id="RIIL{{$no}}" type="text" style="text-align: right"  class="form-control RIIL text-primary">
                                        </td>
...
// 		 mati();
	...
	function hidup() {
		...
		for (i = 0; i <= jumlahdata; i++) {
			...
			$("#AK" + i.toString()).attr("readonly", true);
			$("#RILL" + i.toString()).attr("readonly", false);
	...
	function mati() {
		...
		for (i = 0; i <= jumlahdata; i++) {
			...
			$("#NA_BRG" + i.toString()).attr("readonly", true);
			$("#AK" + i.toString()).attr("readonly", true);
			$("#RILL" + i.toString()).attr("readonly", true);
			$("#QTY" + i.toString()).attr("readonly", true);
    ...
    function tambah() {
		...
		jumlahdata = 100;
		for (i = 0; i <= jumlahdata; i++) {
			$("#AK" + i.toString()).autoNumeric('init', {
				aSign: '<?php echo ''; ?>', vMin: '-999999999.99'});


			$("#RIIL" + i.toString()).autoNumeric('init', {
				aSign: '<?php echo ''; ?>',
				vMin: '-999999999.99'
			});
			...
			$("#SALDO" + i.toString()).autoNumeric('init', {
				aSign: '<?php echo ''; ?>',
				vMin: '-999999999.99'
			});
