...
<div class="content-wrapper">
                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-body">
                            ...
                            <form
                                action="{{ $tipx == 'new' ? url('/uppn_new/store?flagz=' . $flagz . '') : url('/uppn_new/update/' . $header->NO_ID . '&flagz=' . $flagz . '') }}"
                                ...
                                <div class="mt-3 col-md-12 form-group row">
                                    <div class="col-md-4">
                                        <button hidden type="button" id='TOPX'
                                            onclick="location.href=`{{ url('/uppn_new/edit/?idx=' . $idx . '&tipx=top&flagz=' . $flagz . '') }}`"
                                            class="btn btn-outline-primary">Top</button>
                                        <button hidden type="button" id='PREVX'
                                            onclick="location.href=`{{ url('/uppn_new/edit/?idx=' . $header->NO_ID . '&tipx=prev&flagz=' . $flagz . '&buktix=' . $header->NO_BUKTI) }}`"
                                            class="btn btn-outline-primary">Prev</button>
                                        <button hidden type="button" id='NEXTX'
                                            onclick="location.href=`{{ url('/uppn_new/edit/?idx=' . $header->NO_ID . '&tipx=next&flagz=' . $flagz . '&buktix=' . $header->NO_BUKTI) }}`"
                                            class="btn btn-outline-primary">Next</button>
                                        <button hidden type="button" id='BOTTOMX'
                                            onclick="location.href=`{{ url('/uppn_new/edit/?idx=' . $idx . '&tipx=bottom&flagz=' . $flagz . '') }}`"
                                            class="btn btn-outline-primary">Bottom</button>
                                    </div>
                                    <div class="col-md-5">
                                        <button hidden type="button" id='NEWX'
                                            onclick="location.href=`{{ url('/uppn_new/edit/?idx=0&tipx=new&flagz=' . $flagz . '') }}`"
                                        ...
                                        <button hidden type="button" id='UNDOX'
                                            onclick="location.href=`{{ url('/uppn_new/edit/?idx=' . $idx . '&tipx=undo&flagz=' . $flagz . '') }}`"
                                    ...
                                    <div class="col-md-3">
                                        ...
                                        <!-- <button type="button" id='CLOSEX'  onclick="location.href='{{ url('/uppn_new?flagz=' . $flagz . '') }}'" class="btn btn-outline-secondary">Close</button> -->
    ...
    <div class="modal fade" id="browseKirimModal" tabindex="-1" role="dialog"
        aria-labelledby="browseKirimModalLabel" aria-hidden="true">
    ...
    <script>
        ...
        $(document).ready(function() {
                    ...
                    success: function(response) {
                        ...
                        for (i = 0; i < resp.length; i++) {
                            dTableBKirim.row.add([
                                '<a href="javascript:void(0);" onclick="chooseKirim(\'' +
                                resp[i].NO_KIRIM + '\', \'' + resp[i].CBG + '\', \'' + resp[
                                    i].CBG_TUJU + '\' )">' + resp[i].NO_KIRIM + '</a>',
                                resp[i].CBG,
                                resp[i].CBG_TUJU,
                            ]);
                        }
            ...
            chooseKirim = function(NO_KIRIM, CBG, CBG_TUJU) {
            ...
            //     }
            // });
            $("#NO_KIRIM").on('keydown', function(e) {
                if (e.key === 'Enter') {
                    e.preventDefault();
                    let no_kirim = $(this).val();
                    if (no_kirim !== '') {
                        getKirimd(no_kirim, 'BIASA');
                    }
                }
                ...
                success: function(resp) {
                    ...
                    for (i = 0; i < resp.length; i++) {
                        ...
                        html += `<tr>
                                    ...
                                    <td>
                                        <input name='NO_ID[]' id='NO_ID${i}' value='new' type='text' class='NO_ID form-control' hidden>
									...
									<td>
										<input name='QTY_KIRIM[]' onclick='select()' onblur='hitung()' id='QTY_KIRIM${i}' value="${resp[i].QTY}" type='text' style='text-align: right' class='form-control QTY_KIRIM text-primary' readonly >
									</td>
									<td>
										<input name='QTY[]' onclick='select()' onblur='hitung()' id='QTY${i}' value="0" type='text' style='text-align: right' class='form-control QTY text-primary' >
									</td>
									<td>
										<input name='HARGA[]' onclick='select()' onblur='hitung()' id='HARGA${i}' value="${resp[i].HARGA}" type='text' class='form-control  HARGA'>
									</td>
									<td>
										<input name='TOTAL[]' onclick='select()'  id='TOTAL${i}' value="0" type='text' class='form-control  TOTAL'>
									</td>
									<td>
										<input name='KET[]' id='KET${i}' value="" type='text' class='form-control  KET'>
									</td>
                                    <td>
										<input name='SELISIH[]' onclick='select()'  id='SELISIH${i}' value="0" type='text' class='form-control  SELISIH'>
									</td>
        ...
        function tambah_barang() {
            ...
            $(".KD_BRG").each(function() {
                ...
                if ($(this).val() == KD_BRGX1) {
                    let qtySekarang = parseFloat(z.find('.QTY').val().replace(/,/g, '')) || 0;
                    z.find('.QTY').val(qtySekarang + QTYX1);
        ...
        function hitung() {
            ...
            $(".QTY").each(function() {
                ...
                let qty_kirim = parseFloat(z.find('.QTY_KIRIM').val().replace(/,/g, '')) || 0;
                let harga = parseFloat(z.find('.HARGA').val().replace(/,/g, '')) || 0;

                let total = qty * harga;
                let selisih = qty_kirim - qty;

                z.find('.TOTAL').val(numberWithCommas(total));
                z.find('.SELISIH').val(numberWithCommas(selisih));
        ...
        // 	{
        // 		window.location ="{{ url('/uppn_new/delete/' . $header->NO_ID . '/?flagz=' . $flagz . '') }}";
        // 		//return true;
        // 	}
        // 	return false;
        // }
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    Swal.fire({
                        ...
                        loc = "{{ url('/uppn_new/delete/' . $header->NO_ID) }}" + '?flagz=' +
        ...
        function closeTrans() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    loc = "{{ url('/uppn_new/') }}" + '?flagz=' + encodeURIComponent(flagz);
                    window.location = loc;
                } else {
                    Swal.fire({
                        icon: 'info',
        ...
        function CariBukti() {
            ...
            var flagz = "{{ $flagz }}";
            var cari = $("#CARI").val();
            var loc = "{{ url('/uppn_new/edit/') }}" + '?idx={{ $header->NO_ID }}&tipx=search&flagz=' + encodeURIComponent(
