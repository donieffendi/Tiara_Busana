...
class RStockbController extends Controller
{
	...
	public function jasperStockbReport(Request $request) 
	{
		...
		if ($semua == 1){
			...
			$file 	= 'disk conter all';
			$query = DB::SELECT("SELECT AA.cnt as cnt, AA.ncnt as ncnt,'' as kd_brg,'' as na_brg, SUM(AA.qtyx) as qtyx, round(SUM(AA.total_cash)) as total_cash, AA.dis,       
